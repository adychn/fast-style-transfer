...
def ffwd_video(path_in, path_out, checkpoint_dir, use_IN, device_t='/gpu:0', batch_size=4):
    ...
    g = tf.Graph()
    soft_config = v1.ConfigProto(allow_soft_placement=True)
    soft_config.gpu_options.allow_growth = True
    with g.as_default(), g.device(device_t), \
            v1.Session(config=soft_config) as sess:
        ...
        def style_and_write(count):
            ...
            preds = sess.run(ffwd_net, feed_dict={img_placeholder: X})
            for i in range(0, count):
                video_writer.write_frame(np.clip(preds[i], 0, 255).astype(np.uint8))
...
def ffwd(data_in, paths_out, checkpoint_dir, use_IN, device_t='/gpu:0', batch_size=4):
    ...
    with g.as_default(), g.device(device_t), v1.Session(config=soft_config) as sess:
        ...
        # run feed-forward net and save the stylized image
        ...
        for i in range(num_iters):
            ...
            # sess.run(fetches, feed_dict=None, options=None, run_metadata_None)
            preds = sess.run(ffwd_net, feed_dict={img_placeholder:X})
            for j, path_out in enumerate(curr_batch_out):
                save_img(path_out, preds[j])
...
def build_parser():
    ...
    parser.add_argument('--checkpoint', type=str,
                        dest='checkpoint_dir',
                        help='dir or .ckpt file to load checkpoint from (default is ./checkpoint/)',
                        metavar='CHECKPOINT', default=CHECKPOINT_DIR)

    parser.add_argument('--in-path', type=str,
                        dest='in_path',help='dir or files to transform',
                        metavar='IN_PATH', required=True)

    parser.add_argument('--out-path', type=str,
                        dest='out_path', help='dir or files of the transformed',
                        metavar='OUT_PATH', required=True)

    parser.add_argument('--device', type=str,
                        dest='device', help='device to perform compute on (default is on CPU)',
                        metavar='DEVICE', default=DEVICE)

    parser.add_argument('--batch-size', type=int,
                        dest='batch_size', help='batch size for feedforwarding (default is 4)',
...
def main():
    ...
    if not os.path.isdir(opts.in_path): # in_path is a file
        if os.path.exists(opts.out_path) and os.path.isdir(opts.out_path):
            ...
            # out_path = out_path_dir/in_path_basename_file.jjj
            out_path = os.path.join(opts.out_path, image_name)
        else: # put output file in out-path.
            # out_path = out_path_file.jjj
            out_path = opts.out_path

        print(f"Input path is: {opts.in_path}.")
